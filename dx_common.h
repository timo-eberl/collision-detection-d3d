...
void dx_shared_begin_pass(dx_shared_state* sh, const dx_shape* rigids, uint32_t rigid_count,
						  const dx_shape* statics, uint32_t static_count, bool statics_changed,
...
// counter, flushes, and returns the total amount of collisions.
uint32_t dx_shared_execute_and_get_count(dx_shared_state* sh, uint32_t static_count, dx_profile* prof);

// Copies the output into a readback heap, executes the queue, and returns the final malloc'd array
// to the host.
dx_collision* dx_shared_readback_collisions(dx_shared_state* sh, uint32_t count, dx_profile* prof);
